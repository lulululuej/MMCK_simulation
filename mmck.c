...
int main() {
    infile = fopen("mmck.in", "r");
    outfile = fopen("mmck.out", "w");
    num_events = 2; // arrive 1 and depart 2
    
    fscanf(infile, "%d %Lf %Lf %d %d", &num_delays_required, &mean_inter_arr_time, &mean_service_time, &total_num_of_server, &num_cus_in_system);
...
void report(void){
    ...
    fprintf(outfile, "Number of servers%14.3d\n", total_num_of_server);
    fprintf(outfile, "System capacity%16.3d\n", num_cus_in_system);
